...
func AddRenderedReviewDocs(ctx context.Context, dbPath, repoRoot string, strict bool) error {
	...
	for _, doc := range reviewDocs {
		...
		if renderErr != nil {
			diagnostics = append(diagnostics, reviewwidgets.Diagnostic{Severity: "error", Message: renderErr.Error()})
		} else {
			if page.Title != "" {
				title = page.Title
			}
			for _, pageErr := range page.Errors {
				diagnostics = append(diagnostics, reviewwidgets.Diagnostic{Severity: "error", Message: pageErr})
			}
			if strict {
		...
		if strict && len(diagnostics) > 0 {
			...
			return fmt.Errorf("render doc %s: %d diagnostic(s): %s", slug, len(messages), strings.Join(messages, "; "))
		}
